...
**A second issue surfaced deploying migration `0008` itself.** After pushing, the forecast fix (Phase 10, below) was confirmably live — but every `next_display_code()`-dependent write (creating a relocation, a standalone handoff, syncing a survey) started failing with a bare `500 Internal Server Error` on the live Railway backend, across every affected prefix. That pattern — the rest of the deploy clearly running the new code, but every sequence-dependent write uniformly broken — is consistent with migration `0008`'s sequences/grants not actually taking effect against the database the running app connects to, for a reason this session couldn't pin down further without direct access to Railway's own deploy logs (not available in this environment). **Migration `0009`** doesn't guess at the root cause; it re-applies `0008`'s own work idempotently (`create sequence if not exists`, re-`setval`, re-`grant usage`) — a genuine fix if `0008` silently didn't take effect, and a verified no-op (115 tests still pass locally) if it did. If this recurs, the next step is pulling Railway's actual deploy/build logs for the `0008` deploy, which this session had no way to access.
