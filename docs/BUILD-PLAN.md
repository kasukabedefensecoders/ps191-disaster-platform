...
**Found live: the Evacuation Routes screen showed RT-07 for every zone, not a display bug but a real data gap** — `routes` had no `zone_id` at all, and RT-07 was the *only* row ever seeded, so naturally the same route rendered no matter which zone a judge was looking at. **Migration `0012`** adds `zone_id` (backfilling the existing RT-07 row to ZN-01 — the prototype's own "Route RT-07 · Upper Ridge → SH-01" label, Upper Ridge being ZN-01 — then making the column `not null`). `app/seed.py` now seeds one route per zone: ZN-02/03/04 each get a straight-line route to their nearest eligible shelter, computed the same honest way `services/shelter_matching.py` already handles "no OSRM instance" for households (great-circle `ST_DistanceSphere`-equivalent distance, duration from the same 25 km/h rural-hill-road planning speed) — not disguised as road-routed geometry the way RT-07 genuinely is. The frontend (`pages/Routes.tsx`) now fetches zones alongside routes, labels each route card "For ZN-0X · <zone name>", and sorts by zone rather than by route code, so it's visibly obvious each zone has its own distinct route rather than all four coincidentally showing the same one. Pinned by `tests/test_routes_api.py::test_field_officer_sees_seeded_route_no_rls`, which now asserts all four routes exist with distinct distances, not just RT-07 alone.

**Found live, immediately after that fix shipped: distinct routes per zone, but no way to see which shelter any of them actually led to.** `routes` still had no shelter reference at all — `dest_geom` is a bare, unnamed point, and for RT-07 specifically it isn't even reliably at the target shelter's own coordinates (it's wherever the real fetched OSM segment happens to end). **Migration `0013`** adds `shelter_id` (backfilling RT-07 to SH-01, the same prototype label as the `zone_id` backfill), and `app/seed.py` now sets it explicitly for every route — for ZN-02/03/04 it's whichever shelter `_nearest_shelter` already picked as that route's destination, so the same computation now drives both the geometry and the named shelter rather than the shelter being un-nameable. `pages/Routes.tsx` shows it as "→ SH-0X · &lt;shelter name&gt;" on each card. Also fixed here: the three routes created as a live data patch (via `POST /routes`, since the same session hit a garbled-input bug typing a long command into Railway's web console) had picked up the next available sequence codes (`RT-8`/`RT-9`/`RT-10`) instead of matching what `app.seed` would produce fresh (`RT-02`/`RT-03`/`RT-04`) — cosmetic only, but renamed to match via a direct `UPDATE` for consistency with a fresh seed.

**Reported: the straight zone-to-shelter lines "looked wrong next to a real map" — they should follow actual roads.** Rather than wait on the self-hosted OSRM binary this phase never stood up (still true — see TRD §11), `frontend/src/lib/osrm.ts` calls OSRM's free public demo server (`router.project-osrm.org`, no key) directly from the browser: one request per route, in parallel, for every route on the page. Each route's `path`/`distance_km`/`estimated_duration_minutes` are overridden with OSRM's real road-routed figures once the call resolves, cached by `route_id` in component state (routes are baseline data, never recreated by a reset, so this naturally fetches each pair only once per session). Any OSRM failure — timeout, network, rate limit — logs a `console.warn` and falls back per-route to whatever the backend already returned (RT-07's own real pre-fetched OSM stretch, or the straight line for the rest), so a slow or unreachable public API never breaks the page. This is a pure frontend change with no backend/migration involved.

**Reported, next: all four routes drawn simultaneously in the same colour, overlapping into an unreadable tangle, with no way to tell start from end.** `pages/Routes.tsx` now tracks a single selected route (clicking a card selects it; defaults to the first in list order) and passes `RouteMap` only that one, auto-fit to it, instead of the whole list at once. `RouteMap` also gained origin/destination markers — an amber dot at the route's start, a green house-badge at its end — built as Leaflet `divIcon`s (inline HTML, no external image assets, avoiding the classic missing-marker-icon problem default `L.Icon`s hit in bundler setups like this one).

**Found live immediately after that: markers moved to the newly-selected route's zone/shelter correctly, but for every route except RT-07 the line didn't reach them — an orphaned marker with no connecting line.** Not a stale-state or caching bug (verified directly against OSRM's own response: each route's fetched geometry, distance, and both waypoints were genuinely its own, not reused from another route). The real cause: markers were placed at `route.origin_geom`/`dest_geom` — the raw zone centroid and shelter point — while the *drawn line* uses OSRM's road-snapped geometry, and OSRM snaps an off-road demo centroid to the nearest real road, which can be several km away (confirmed via OSRM's own `waypoints[].distance`: ZN-02's centroid snaps 5.05 km from the nearest road, ZN-04's 1.54 km). RT-07 never showed this because its origin marker was already built from real OSM road coordinates, not a raw centroid. Fixed by placing both markers at `pathLatLngs(route.path)`'s own first/last points — the actual drawn line's two ends — instead of a separately-stored coordinate, which guarantees a marker is always exactly on its route's line regardless of geometry source.

**Extended to Logistics Tracker: "View Route" used to just `navigate("/routes")`,** landing on Evacuation Routes with whatever route happened to be selected — unrelated to the vehicle clicked, and most vehicles don't map onto an existing zone→shelter `Route` row anyway (`demo_seed.py`'s `VEHICLE_PLAN` consolidates households from different zones onto one bus — e.g. BUS-03 carries one ZN-03 and one ZN-04 household — and can send them to a different shelter than that zone's own evacuation route uses, e.g. BUS-04 → SH-03 while ZN-03's own route, RT-03, goes to SH-01). `components/VehicleRouteModal.tsx` reuses `RouteMap` and `lib/osrm.ts`'s `fetchOsrmRoute` directly rather than building a second implementation: it computes the vehicle's pickup point (the distinct zones its households come from, averaged — households in the same zone share that zone's own seeded point) and the real shelter point, fetches one fresh OSRM route between them, and renders it with the same road-following line and start/shelter markers as Evacuation Routes, in a modal on the Logistics Tracker page itself. Verified against BUS-01 (single zone) and BUS-03 (two zones, ZN-03 + ZN-04).
...
**A second issue surfaced deploying migration `0008` itself — root cause since confirmed via Railway's dashboard.** After pushing, the forecast fix (Phase 10, below) was confirmably live — but every `next_display_code()`-dependent write (creating a relocation, a standalone handoff, syncing a survey) started failing with a bare `500 Internal Server Error` on the live Railway backend. **Migration `0009`** was written as a defensive, idempotent re-application of `0008`'s own DDL without knowing the exact cause; it's a correct fix either way but wasn't the actual missing piece. The real cause, found afterward by opening Railway's dashboard directly: **the service's Custom Start Command (Settings → Deploy) silently overrides `railway.json` and has never run `alembic upgrade head` at all** — the database had been sitting at revision `0007` since the day it was first migrated by hand, with every deploy since just restarting `uvicorn` against whatever revision was already there. Confirmed by running `alembic upgrade head` manually via Railway's Console: it applied `0007 -> 0008 -> 0009` for the first time, and every previously-failing endpoint started working immediately. See `docs/TRD.md` §11's incident note for the full account, including a failed attempt to fix the Custom Start Command properly (it caused a ~1 hour outage — the container hangs indefinitely when `alembic upgrade head &&` is prepended, for a reason not yet diagnosed) and the current state: **migrations must be applied by hand via Railway's Console after every deploy that adds one, until that startup hang is understood.**
...
**Found live, reported by name: GSI/risk colour mismatch on the Red Zone Map after "Reset demo data."** A zone GSI marks "High" was displaying green/low-risk on the map. Cause: the seeded `risk_score_72h` (at that point correctly matching `ml/forecast_model.py`'s own 72-hour training-curve output) is, for every zone in this district, the *low* end of a curve that peaks at 6-24h and recedes by 72h — a genuine property of the trained model, not a bug in it, but a bad choice for what the map shows as the *default* risk badge before anyone has touched the forecast feature. Fixed by decoupling the two: `app/seed.py`'s `ZONES` now carries a GSI-derived demo baseline (`gsi_score/100` plus a small offset — 0.72/0.75/0.50/0.52 for ZN-01..04) instead of the model-exact value, chosen specifically to land in the same `riskColor.ts` band as `gsi_classification` (High → sev1/sev2 red-orange, Moderate → sev3 amber). `gsi_classification`/`gsi_score` themselves are untouched — still the real, static, dataset-sourced GSI classification. Clicking "Generate forecast" still calls the real model and can legitimately show a different, lower number afterward; only the untouched default is curated. A new test (`tests/test_zones_api.py::test_seeded_risk_score_lands_in_the_same_colour_band_as_gsi_classification`) pins this invariant. Since `risk_score_72h` also feeds `prio_score` (32% weight), this shifted several households' priority tiers again — `tests/test_scoring.py`'s `EXPECTED` table carries the third revision, with its module docstring recording all three.
...
CV differencing/thresholding on one curated before/after Sentinel pair. Zero dependency on the scoring/data-model work — originally needs only MinIO (Phase 0) — so it can be built by whoever's free earliest rather than waiting its turn.

**Found live: "Run detection" 400'd for ZN-01 right after "Reset demo data."** Not caused by reset — it never touches `change_detections` or the image store — but by a compounding pair of gaps: (1) `app/seed.py` only ever generated a curated pair for ZN-01, once, at initial setup, so no other zone ever had one; and (2) no MinIO instance exists on the live Railway deployment at all (`docs/TRD.md` §11's already-recorded gap), so `object_exists()` silently returning `False` for "not reachable" was indistinguishable from "never uploaded." **Migration `0010`** moves the curated pair into Postgres directly (a `sample_imagery` table — two small ~200x200 grayscale PNGs, well within reason for a row) rather than provisioning MinIO on Railway, which this session was not going to attempt again after the migration-deploy incident above. `services/demo_seed.py`'s reset flow now calls `ensure_sample_imagery_for_zone` for every zone before reseeding (not just ZN-01), and `app/seed.py`'s one-time setup does the same, so "Run detection" works immediately for any zone after either a fresh seed or a reset — pinned by `tests/test_change_detection.py::test_reset_demo_data_reseeds_imagery_so_change_detection_works_on_any_zone`.

**Found live, one click later: making imagery always-present exposed a second bug — the screen showed a zone's before/after slider and result panel just because imagery/a past detection *existed*, not because the user had run detection this cycle.** Two independent effects compounded here: `frontend/src/pages/ChangeDetection.tsx` gated the slider on `hasImagery` (bytes present in `sample_imagery`) rather than on a detection actually having been run — meaning as soon as every zone got a curated pair (the fix directly above), every zone's slider rendered unconditionally on page load; and `services/demo_seed.py`'s reset never deleted `change_detections` rows (only `risk_forecasts`/`relocation_records`/`surveys` got that treatment), so a zone a judge had already run detection on in an earlier cycle kept showing that stale result straight through a reset. Fixed on both sides: **migration `0011`** grants `app_user` `DELETE` on `change_detections` (it never had one, same gap migration `0007` closed for `risk_forecasts`), a new `_clear_change_detection_state` deletes those rows (and nulls the now-dangling `incident_outcomes.detection_id` FK first) as part of every reset, and the frontend now renders a zone's imagery/result/honest-scope panel only when a `change_detections` row exists for it (`latest`) — a zone with no row renders a single-line "not run yet this demo cycle" card with just the zone name and the Run button, and images for that zone aren't even fetched until it has one. Pinned by `tests/test_change_detection.py::test_reset_demo_data_clears_previously_run_detections`.
