...
**Routing engine, what's actually shipped.** The row above is the designed architecture — a self-hosted OSRM instance in the Docker Compose stack — and that's still what §13's local-Compose environment describes. The two screens that actually need road-following geometry (Evacuation Routes' map, and Logistics Tracker's "View Route") don't call a self-hosted OSRM at all: `frontend/src/lib/osrm.ts` calls OSRM's free public demo server (`router.project-osrm.org`) directly from the browser, client-side, with no key and no backend involvement. This sidesteps the "no OSRM instance provisioned" gap (§11) entirely for those two screens — the line drawn, and the distance/duration shown, are genuine road-routed OSRM output, not a synthetic straight line — but it's a different mechanism than the row above describes, is out of scope for `services/shelter_matching.py`'s own routing (still the straight-line PostGIS proxy noted in §11), and depends on a shared public resource with no SLA (falls back to a straight line, logging a `console.warn`, if it's ever unreachable or rate-limited).
...
- **Backend (Railway):** the root-level `Dockerfile` (a plain `python:3.11-slim` image installing `backend/requirements.txt`, distinct from `backend/Dockerfile`'s docker-compose-scoped build context) builds the FastAPI app. `railway.json` *specifies* a start command of `alembic upgrade head && uvicorn app.main:app --host 0.0.0.0 --port $PORT`, but **the actual live start command is a Custom Start Command set in Railway's own dashboard (Settings → Deploy), which overrides `railway.json` and does not run migrations at all** — `python -m uvicorn app.main:app --host 0.0.0.0 --port 8000` (no `alembic`, hardcoded port rather than `$PORT`). This drift is the real reason migrations 0008/0009 (§14 below and `docs/BACKEND-SCHEMA.md`) never auto-applied and had to be run by hand via Railway's Console — see the incident note below before touching this again. Railway supplies its own managed Postgres (PostGIS-enabled) and injects `DATABASE_URL`/`MIGRATION_DATABASE_URL` as plain `postgresql://` URLs; `Settings._normalize_db_url` (`backend/app/config.py`) rewrites either that or a bare `postgres://` scheme to `postgresql+psycopg://` so SQLAlchemy doesn't fall back to the unavailable psycopg2 driver. Redis, MinIO and OSRM are not part of this deployment — see the gaps note below.
...
**Known gaps in the live pilot deployment** (all “designed for, not built” in this specific environment, distinct from the production gaps below): no Redis instance is provisioned, so the caching/live-feed-buffering role §3/§4 describe is unused at this stage; no MinIO/S3 bucket is wired in, so `photo_url` remains a reference without a live object store behind it in this deployment (the change-detection demo's `before_image_ref`/`after_image_ref` no longer need one — migration `0010`, Backend Schema §5.12a — moved that one curated pair into Postgres specifically because this gap made it unreachable); and no *self-hosted* OSRM instance is deployed anywhere, local or hosted, matching the gap originally recorded in `docs/BUILD-PLAN.md` Phase 7. That gap no longer means "no real routing" for a user, though: Evacuation Routes and Logistics Tracker's "View Route" both draw genuine OSRM road-routed paths, fetched client-side straight from OSRM's public demo server (§4's routing-engine note above) — no backend or self-hosted engine needed for those two screens. What's still true from the original gap: `services/shelter_matching.py`'s own distance/duration remains the straight-line PostGIS proxy (no routing engine, self-hosted or public, feeds that calculation), and the `routes` table's seeded `distance_km`/`estimated_duration_minutes` baseline (used until a route is actually opened and OSRM overrides it client-side) is still the same great-circle estimate as before.

**Migrations do not auto-apply on this deployment — incident record.** Discovered when migration 0008 shipped but every `next_display_code()`-dependent write started 500ing live: the database was still sitting at revision 0007, because the dashboard's Custom Start Command (see above) has never run `alembic upgrade head`. Fixed for that specific gap by running the migration by hand via Railway's Console (`alembic upgrade head`, confirmed: `0007 -> 0008 -> 0009`). **A follow-up attempt to fix this properly — adding `alembic upgrade head &&` back to the Custom Start Command — caused the live container to hang indefinitely on startup** (it connects to Postgres and logs `Context impl PostgresqlImpl` / `Will assume transactional DDL`, then never proceeds to start the server; reproducible on two separate deploys, ~1 hour of downtime, root cause not identified — the identical command run interactively in the same container's Console completes in seconds). The Custom Start Command has been reverted to the original migration-less version to restore service. **Net effect: the database is correctly migrated right now, but every future migration needs to be applied by hand** (`railway` CLI or dashboard Console → `alembic upgrade head`) **until someone can reproduce and fix the startup hang** — don't re-attempt the `&&` fix without a way to inspect why the process stalls (Railway support, or a locally-reproduced environment matching Railway's exact container runtime, neither of which this session had).
...
- Resolved risk: OSRM's road-network extract for Dima Hasao turning out too sparse in OpenStreetMap coverage. Verified early (`docs/BUILD-PLAN.md` Phase 7) via a live Overpass query — real NH27/NH627 national-highway geometry runs directly through the Haflong area, no manual road-graph patch needed. The self-hosted OSRM engine itself was never stood up regardless (§4, §11); the two screens that need road-following geometry route around that separately by calling OSRM's public demo API client-side, which draws on the same underlying OSM data and confirms the coverage question moot for this district either way.
